...
import com.michalplachta.cats.free.ServerDSL.{ GetOpponentFor, SendDecision, Server }

import scala.concurrent.duration._
import scala.concurrent.Future

object LocalServerInterpreter extends (Server ~> Future) {
  final case class GetVerdict(prisoner: Prisoner, otherPrisoner: OtherPrisoner, decision: Decision)
  ...
  def apply[A](i: Server[A]): Future[A] = i match {
    ...
    case SendDecision(prisoner, otherPrisoner, decision) ⇒
      server.ask(GetVerdict(prisoner, otherPrisoner, decision))(5.seconds).mapTo[Verdict]
  }
