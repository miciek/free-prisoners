...
object MultiplayerServer extends App {
  final case class PrisonerPlayer(name: String, actorRef: ActorRef, decision: Option[Decision])
  ...
  class Server extends Actor with ActorLogging {
    ...
    def receive: Receive = {
      ...
      case (playerName: String, opponentName: String, decisionStr: String) ⇒
        def playerWithDecision(player: Option[PrisonerPlayer], opponent: Option[PrisonerPlayer], playerDecision: Decision) =
          if (player.exists(_.name == playerName)) player.map(_.copy(decision = Some(playerDecision))) else player
        ...
        log.info(s"Received decision of $playerName against $opponentName: $decisionStr ($playerDecision)")
    }

    private def announcePlayers(): Unit = {
      def announcePlayer(maybePlayer: Option[PrisonerPlayer], maybeOpponent: Option[PrisonerPlayer]) = for {
        player ← maybePlayer
    ...
    private def endGameIfAllDecisionsReceived(): Unit =
      if (playerA.exists(_.decision.isDefined) && playerB.exists(_.decision.isDefined)) endGame()
    ...
    private def resolveGame(): Unit = {
      def decision(player: Option[PrisonerPlayer]) = player.flatMap(_.decision).getOrElse(Silence)
      def sendVerdict(player: Option[PrisonerPlayer], opponent: Option[PrisonerPlayer]) =
        player.foreach { _.actorRef ! verdict(decision(player), decision(opponent)).years }
